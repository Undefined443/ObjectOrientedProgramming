    ...
    if (!status) {  // if elevator is not running
        ...
        if (get_time_gap() > speed * time_unit) {
            ...
            if (next_floor < 0 || next_floor >= floors.size()) {
                throw std::out_of_range("elevator " + std::to_string(id) + ": floor index (" +
                                        std::to_string(next_floor) + ") out of range.");
            }
...
void elevator::board() {
    ...
    while (!boarding_queue.empty() && boarding_queue.front()->get_current_elevator() != nullptr &&
           boarding_queue.front()->get_current_elevator() != this) {
...
void elevator::alight() {
    ...
    for (auto iter = registry[current_floor].begin(); iter != registry[current_floor].end();
         ++iter) {  // scan the registry and alight passengers
...
int elevator::choose_direction() {
    ...
    for (auto &p : registry) {  // scan the registry
        if (!p.second.empty()) {
            ...
            if ((dest_flr - crt_flr) * direction >
                0) {               // destination floor is at the same direction as the elevator is going
...
bool elevator::is_full() const { return full; }

bool elevator::is_accessible(int floor_id) {
    return std::any_of(accessible_floors.cbegin(), accessible_floors.cend(),
                       [floor_id](auto &floor) { return floor->get_id() == floor_id; });
}

// Setters
void elevator::set_current_floor(class floor *f) { current_floor = f; }

void elevator::set_floors(std::vector<class floor *> &floors) { this->floors = floors; }

// set floors reachable by the elevator, invoke before starting the elevator
void elevator::add_accessible_floor(class floor *f) { accessible_floors.push_back(f); }
...
void elevator::set_monitor(monitor *m) { mon = m; }

void elevator::set_group_id(int g_id) { group_id = g_id; }

void elevator::set_group(std::vector<elevator *> &g) { group = g; }

// Getters
class floor *elevator::get_current_floor() { return current_floor; }

int elevator::get_id() const { return id; }

int elevator::get_direction() const { return direction; }
...
bool elevator::get_status() const { return status; }

int elevator::get_ding_stage() const { return ding_stage; }

int elevator::get_alighting_num(class floor *f) {
    return std::accumulate(registry[f].begin(), registry[f].end(), 0,
                           [f](int sum, passenger *p) { return sum + (p->get_destination() == f->get_id()); });
}

int elevator::get_free_space() const { return conf["elevator.capacity"].get<int>() - int(passengers.size()); }

int elevator::get_load() const { return int(passengers.size()); }

std::vector<class floor *> elevator::get_accessible_floors() const { return accessible_floors; }
...
int elevator::get_group_id() const { return group_id; }

std::vector<elevator *> elevator::get_group() const { return group; }
