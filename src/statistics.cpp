...
void statistics::add_elevator_idle_time(int elevator, long long time) {
    ...
    if (elevator_statistics[elevator][0][0] <
        36000) {  // we need to refresh faster at the beginning, to have a fluent animation
...
void statistics::add_elevator_running_time(int elevator, long long time) {
    ...
    if (elevator_statistics[elevator][1][0] <
        36000) {  // we need to refresh faster at the beginning, to have a fluent animation
...
void statistics::add_passenger_waiting_time(int elevator, std::pair<long, long> time) {
    ...
    auto time_division = (start_time - base_timestamp) * ratio / sampling_interval_millisecond *
                         sampling_interval_millisecond;                         // round down
...
QVector<long long> statistics::get_passenger_statistics() const {
    ...
    result[0] = std::accumulate(passenger_statistics.begin(), passenger_statistics.end(), 0LL) /
                (long long)passenger_statistics.size();
...
void statistics::set_base_timestamp(long long time_stamp) { base_timestamp = time_stamp; }

void statistics::set_time_unit(int _time_unit) { time_unit = _time_unit; }
...
bool statistics::load() {
    ...
    estimated_waiting_time =
        data.get<std::vector<std::map<long long, std::pair<long long, long long>>>>();  // get estimated waiting time
    return true;
}
...
bool statistics::is_rush_hour(int elevator, long long time) {
    if (elevator <= 0 || elevator > estimated_waiting_time.size()) {
        throw std::out_of_range("in statistics.cpp, is_rush_hour: parameter time (" + std::to_string(elevator) +
                                ") out of range.");
    }
    auto ratio = 1000 / time_unit;  // acceleration ratio
    auto relative_time = (time - base_timestamp) * ratio / sampling_interval_millisecond *
                         sampling_interval_millisecond;  // calculate relative time
    auto start_time =
        relative_time - 2 * sampling_interval_millisecond;  // start time, 2 time division before relative time
